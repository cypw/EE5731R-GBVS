function top = colors(bottom, param)
...
for i_img = 1:img_num
    ...
    for i_level = 1:img_level_num      
        ...
        im_fea = zeros([size(im,1), size(im,2), 4]);
        im_map = zeros([map_size(1:2), 4]);
        if size(im,3) == 1
            im = cat(3,im,im,im);
        end
        
        % process - r g b
        im_fea(:,:,1) = im(:,:,1) - (im(:,:,2) + im(:,:,3))./2;
        im_fea(:,:,2) = im(:,:,2) - (im(:,:,1) + im(:,:,3))./2;
        im_fea(:,:,3) = im(:,:,3) - (im(:,:,1) + im(:,:,2))./2;
